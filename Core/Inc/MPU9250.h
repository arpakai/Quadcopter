...
public:
    ...
    bool setup(const uint8_t addr, I2C_HandleTypeDef *hi2c1, UART_HandleTypeDef *huart4, const MPU9250Setting &mpu_setting = MPU9250Setting())
    {
        // Save the I2C and UART handlers
        _hi2c1 = hi2c1;
        _huart4 = huart4;
        ...
        if ((addr < MPU9250_DEFAULT_ADDRESS) || (addr > MPU9250_DEFAULT_ADDRESS + 7))
        {
            ...
            {
                char msg[100];
                snprintf(msg, sizeof(msg), "I2C address 0x%02X is not valid for MPU. Please check your I2C address.\r\n", addr);
                HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
            }
            return false;
        }
        mpu_i2c_addr = addr << 1;
        ...
        if (isConnectedMPU9250())
        {
            ...
            // else
            // {
            //     if (b_verbose && _huart4 != nullptr)
            //     {
            //         char msg[] = "Could not connect to AK8963\r\n";
            //         HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
            //     }
            //     has_connected = false;
            //     return false;
        ...
        else
        {
            if (b_verbose && _huart4 != nullptr)
            {
                char msg[] = "Could not connect to MPU9250\r\n";
                HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
            }
            has_connected = false;
            return false;
    ...
    bool isConnectedMPU9250()
    {
        ...
        {
            char msg[50];
            snprintf(msg, sizeof(msg), "MPU9250 WHO AM I = 0x%02X\r\n", c);
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
        }
    ...
    bool isConnectedAK8963()
    {
        ...
        {
            char msg[50];
            snprintf(msg, sizeof(msg), "AK8963 WHO AM I = 0x%02X\r\n", c);
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
        }
    ...
    void print_yaw_pitch_roll()
    {
        ...
        // Format the string
        len = snprintf(buffer, sizeof(buffer), "Yaw, Pitch, Roll: %.2f, %.2f, %.2f\r\n", getYaw(), getPitch(), getRoll());

        // Transmit the string
        HAL_UART_Transmit(_huart4, (uint8_t *)buffer, len, 1);
    }
...
private:
    ...
    void initAK8963()
    {
        ...
        mag_bias_factory[0] = (float)(raw_data[0] - 128) / 256.0 + 1.0; // Return x-axis sensitivity adjustment values, etc.
        mag_bias_factory[1] = (float)(raw_data[1] - 128) / 256.0 + 1.0;
        mag_bias_factory[2] = (float)(raw_data[2] - 128) / 256.0 + 1.0;
        write_byte(AK8963_ADDRESS, AK8963_CNTL, 0x00); // Power down magnetometer
        HAL_Delay(10);
        ...
        {
            char msg[150];
            snprintf(msg, sizeof(msg), "Mag Factory Calibration Values:\r\n");
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);

            snprintf(msg, sizeof(msg), "X-Axis sensitivity offset value: %.2f\r\n", mag_bias_factory[0]);
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);

            snprintf(msg, sizeof(msg), "Y-Axis sensitivity offset value: %.2f\r\n", mag_bias_factory[1]);
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);

            snprintf(msg, sizeof(msg), "Z-Axis sensitivity offset value: %.2f\r\n", mag_bias_factory[2]);
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
        }
    }
...
private:
    ...
    void calibrate_mag_impl()
    {
        ...
        if (b_verbose && _huart4 != nullptr)
        {
            ...
            // Print calibration done message
            snprintf(msg, sizeof(msg), "Mag Calibration done!\r\n");
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);

            // Print mag biases
            snprintf(msg, sizeof(msg), "AK8963 mag biases (mG)\r\n%.2f, %.2f, %.2f\r\n", mag_bias[0], mag_bias[1], mag_bias[2]);
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);

            // Print mag scale
            snprintf(msg, sizeof(msg), "AK8963 mag scale (mG)\r\n%.2f, %.2f, %.2f\r\n", mag_scale[0], mag_scale[1], mag_scale[2]);
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
        }
    ...
    void collect_mag_data_to(float *m_bias, float *m_scale)
    {
        ...
        {
            char msg[100];
            snprintf(msg, sizeof(msg), "Mag Calibration: Wave device in a figure eight until done!\r\n");
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
        }
        HAL_Delay(4000);
        ...
        {
            char msg[100];
            snprintf(msg, sizeof(msg), "mag x min/max:\r\n%d\r\n%d\r\n", mag_min[0], mag_max[0]);
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
            snprintf(msg, sizeof(msg), "mag y min/max:\r\n%d\r\n%d\r\n", mag_min[1], mag_max[1]);
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
            snprintf(msg, sizeof(msg), "mag z min/max:\r\n%d\r\n%d\r\n", mag_min[2], mag_max[2]);
            HAL_UART_Transmit(_huart4, (uint8_t *)msg, strlen(msg), HAL_MAX_DELAY);
        }
    ...
    bool self_test_impl() // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
    {
        ...
        {
            char buf[100];
            snprintf(buf, sizeof(buf), "x-axis self test: acceleration trim within : %.1f%% of factory value\n", self_test_result[0]);
            HAL_UART_Transmit(_huart4, (uint8_t *)buf, strlen(buf), HAL_MAX_DELAY);

            snprintf(buf, sizeof(buf), "y-axis self test: acceleration trim within : %.1f%% of factory value\n", self_test_result[1]);
            HAL_UART_Transmit(_huart4, (uint8_t *)buf, strlen(buf), HAL_MAX_DELAY);

            snprintf(buf, sizeof(buf), "z-axis self test: acceleration trim within : %.1f%% of factory value\n", self_test_result[2]);
            HAL_UART_Transmit(_huart4, (uint8_t *)buf, strlen(buf), HAL_MAX_DELAY);

            snprintf(buf, sizeof(buf), "x-axis self test: gyration trim within : %.1f%% of factory value\n", self_test_result[3]);
            HAL_UART_Transmit(_huart4, (uint8_t *)buf, strlen(buf), HAL_MAX_DELAY);

            snprintf(buf, sizeof(buf), "y-axis self test: gyration trim within : %.1f%% of factory value\n", self_test_result[4]);
            HAL_UART_Transmit(_huart4, (uint8_t *)buf, strlen(buf), HAL_MAX_DELAY);

            snprintf(buf, sizeof(buf), "z-axis self test: gyration trim within : %.1f%% of factory value\n", self_test_result[5]);
            HAL_UART_Transmit(_huart4, (uint8_t *)buf, strlen(buf), HAL_MAX_DELAY);
        }
    ...
    uint8_t read_byte(uint8_t address, uint8_t subAddress)
    {
        ...
        HAL_StatusTypeDef i2c_err_ = HAL_I2C_Mem_Read(_hi2c1, address, subAddress, I2C_MEMADD_SIZE_8BIT, &data, 1, HAL_MAX_DELAY);
    ...
    {
        HAL_StatusTypeDef i2c_err_ = HAL_I2C_Mem_Read(_hi2c1, address, subAddress, I2C_MEMADD_SIZE_8BIT, dest, count, HAL_MAX_DELAY);
    ...
    void print_i2c_error()
    {
        ...
        char buf[50];
        snprintf(buf, sizeof(buf), "I2C ERROR CODE : %d\n", i2c_err_);
        HAL_UART_Transmit(_huart4, (uint8_t *)buf, strlen(buf), HAL_MAX_DELAY);
    }
};
